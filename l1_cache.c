...
{   
  ...
  for(int i=0; i<L1_NUM_CACHE_SETS; i++){
    for(int j=0; j<4; j++){
      l1_cache[i].lines[j].v_r_d_tag &= ~(L1_VBIT_MASK);     
    }
                     ...
                     uint8_t control, uint32_t *read_data, uint8_t *status)
{
  ...
  uint32_t address_index = (address & L1_SET_INDEX_MASK) >> L1_SET_INDEX_SHIFT;
 
 
  uint32_t address_tag = (address & L1_ADDRESS_TAG_MASK) >> L1_ADDRESS_TAG_SHIFT;
  
  
  uint32_t word_offset = (address & WORD_OFFSET_MASK) >> WORD_OFFSET_SHIFT;
  
    

  int count = 0;
  for(int i=0; i<4; i++){
    if( (l1_cache[address_index].lines[i].v_r_d_tag & L1_VBIT_MASK) 
      && ((l1_cache[address_index].lines[i].v_r_d_tag & L1_ENTRY_TAG_MASK)) == address_tag ){
          count += 1;

          l1_cache[address_index].lines[i].v_r_d_tag |= L1_RBIT_MASK;
          *status |= L1_CACHE_HIT_MASK;

          if (control & READ_ENABLE_MASK){
            
            *read_data = l1_cache[address_index].lines[i].cache_line[word_offset];
            
          }

          if (control & WRITE_ENABLE_MASK){
           
            l1_cache[address_index].lines[i].cache_line[word_offset] = write_data;
            l1_cache[address_index].lines[i].v_r_d_tag |= L1_DIRTYBIT_MASK;
                    ...
                    uint8_t *status)
{
  ...
  // and write the new tag to the entry. 
    
    if (!(l1_cache[address_index].lines[i].v_r_d_tag & L1_VBIT_MASK)) {
        for(int j=0; j<8; j++){
          l1_cache[address_index].lines[i].cache_line[j]= write_data[j];
        }
        l1_cache[address_index].lines[i].v_r_d_tag |= (L1_VBIT_MASK);
        l1_cache[address_index].lines[i].v_r_d_tag &= ~(L1_DIRTYBIT_MASK);
        l1_cache[address_index].lines[i].v_r_d_tag &= ~(L1_RBIT_MASK);
        l1_cache[address_index].lines[i].v_r_d_tag &= ~L1_ENTRY_TAG_MASK;
        l1_cache[address_index].lines[i].v_r_d_tag |= address_tag;
        *status &= ~(L1_CACHE_HIT_MASK);
        
        return;
    }
    else if ( !(l1_cache[address_index].lines[i].v_r_d_tag & L1_RBIT_MASK) 
            && !(l1_cache[address_index].lines[i].v_r_d_tag & L1_DIRTYBIT_MASK) ){
              if(r0_d0_index == ~0x0)
                r0_d0_index = i;
  ...
  uint32_t evicted_index;
  if (r0_d0_index != ~0x0){
    evicted_index = r0_d0_index;
  }
  else if (r0_d1_index != ~0x0){
    evicted_index  = r0_d1_index;
  }
  else if (r1_d0_index != ~0x0){
    evicted_index  = r1_d0_index;
  }
  else{
    evicted_index  = 0x0;
  }
  ...
  if (l1_cache[address_index].lines[evicted_index].v_r_d_tag & L1_DIRTYBIT_MASK){
      uint32_t evicted_entry_tag = l1_cache[address_index].lines[evicted_index].v_r_d_tag & L1_ENTRY_TAG_MASK;
      *evicted_writeback_address = (evicted_entry_tag << L1_ADDRESS_TAG_SHIFT) | (address_index << L1_SET_INDEX_SHIFT);
      for (int i = 0; i<8; i++){
         evicted_writeback_data[i] = l1_cache[address_index].lines[evicted_index].cache_line[i];
      }
      *status |= 0x1;
  }
  else{
  ...
  for(int i=0; i<8; i++){
      l1_cache[address_index].lines[evicted_index].cache_line[i] = write_data[i];
  }
  l1_cache[address_index].lines[evicted_index].v_r_d_tag |= L1_VBIT_MASK;
  l1_cache[address_index].lines[evicted_index].v_r_d_tag &= ~L1_DIRTYBIT_MASK;
  l1_cache[address_index].lines[evicted_index].v_r_d_tag &= ~L1_RBIT_MASK;
  l1_cache[address_index].lines[evicted_index].v_r_d_tag &= ~L1_ENTRY_TAG_MASK;
  l1_cache[address_index].lines[evicted_index].v_r_d_tag |= address_tag;
...
void l1_clear_r_bits()
{
  for(int i=0; i<L1_NUM_CACHE_SETS; i++){
    for(int j=0; j<4; j++){
      l1_cache[i].lines[j].v_r_d_tag &= ~L1_RBIT_MASK;
      }
