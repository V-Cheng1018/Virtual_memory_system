/***********************************************************
  ...
  1MB direct-mapped, write-back cache.
   ...
   32-bit address is decomposed as follows:
                     ...
                     uint8_t control, uint32_t read_data[], uint8_t *status)
{
  ...
  uint32_t address_index = (address & L2_INDEX_MASK ) >> L2_INDEX_SHIFT;
  

  
  uint32_t address_tag = (address & L2_ADDRESS_TAG_MASK) >> L2_ADDRESS_TAG_SHIFT;
  
  if (!(l2_cache[address_index].v_d_tag & L2_VBIT_MASK) 
        || (l2_cache[address_index].v_d_tag & L2_ENTRY_TAG_MASK) != address_tag){
  ...
  //the cache line index and set the dirty bit. 
  else{
    ...
    if (control & WRITE_ENABLE_MASK){
       ...
       l2_cache[address_index].v_d_tag |= L2_DIRTYBIT_MASK;
    }
                    ...
                    uint8_t *status)
{
  ...
  if (!(l2_cache[address_index].v_d_tag & L2_VBIT_MASK)  
      ...
      l2_cache[address_index].v_d_tag |= L2_VBIT_MASK;
      l2_cache[address_index].v_d_tag &= ~(L2_DIRTYBIT_MASK);
      
      l2_cache[address_index].v_d_tag &= ~L2_ENTRY_TAG_MASK;
      l2_cache[address_index].v_d_tag |= address_tag;
  ...
  else if( (l2_cache[address_index].v_d_tag & L2_VBIT_MASK) 
        &&  l2_cache[address_index].v_d_tag & L2_DIRTYBIT_MASK ){

        uint32_t evicted_entry_tag = l2_cache[address_index].v_d_tag & L2_ENTRY_TAG_MASK ;

        *evicted_writeback_address = (evicted_entry_tag << L2_ADDRESS_TAG_SHIFT) | (address_index << L2_INDEX_SHIFT);
        for (int i=0; i<8; i++){
