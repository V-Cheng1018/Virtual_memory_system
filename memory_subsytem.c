...
void memory_subsystem_initialize(uint32_t memory_size_in_bytes)
{
  ...
  l2_initialize(memory_size_in_bytes);
  l1_initialize(memory_size_in_bytes);
...
void memory_handle_l1_miss(uint32_t address)  
{
  ...
  uint32_t hold_cache_line[8];
  uint8_t status;

  l2_cache_access(address,NULL,READ_ENABLE_MASK,hold_cache_line,&status);
  ...
  uint32_t evicted_write_back_address =0;
  uint32_t evicted_write_back_data[8];
  l1_insert_line(address,hold_cache_line,&evicted_write_back_address,evicted_write_back_data,&status);
  ...
  //      
  if(status & 0x1 ){
    l2_cache_access(evicted_write_back_address,evicted_write_back_data,WRITE_ENABLE_MASK,NULL,&status);
    if (!(status & 0x1)){
        memory_handle_l2_miss(evicted_write_back_address,WRITE_ENABLE_MASK);
        l2_cache_access(evicted_write_back_address,evicted_write_back_data,WRITE_ENABLE_MASK,NULL,&status);
    }
  }
...
void memory_handle_l2_miss(uint32_t address, uint8_t control)
{
  ...
  //l2_insert_line to insert the cache line data in cache_line,
  ...
  uint32_t evicted_write_back_address =0;
  uint32_t evicted_write_back_data [8];
  l2_insert_line(address,cache_line,&evicted_write_back_address,evicted_write_back_data,&status);
  ...
  if(status & 0x1)
    main_memory_access(evicted_write_back_address,evicted_write_back_data,WRITE_ENABLE_MASK,NULL);
